...
import { COLOR_PRIMARY, COLOR_PRIMARY_DARK, COLOR_VIOLET, COLOR_SUCCESS, COLOR_WARNING, COLOR_DANGER, COLOR_INDIGO } from '../constants';
...
export function DashboardScreen({ navigate }: { navigate: (targetScreen: Screen) => void }) {
  ...
  const disciplineStatusColor = totalViolationPoints <= 20
    ? COLOR_SUCCESS
  ...
  const getStatusBadgeStyle = (statusName: string) => {
    ...
    if (normalizedStatus === 'disetujui' || normalizedStatus === 'approved') return { backgroundColor: '#E6F5F3', textColor: COLOR_PRIMARY };
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: '#F1F5F9' }}>
      <div style={{ background: `linear-gradient(135deg, #153D4C 0%, ${COLOR_PRIMARY} 55%, ${COLOR_PRIMARY_DARK} 100%)`, padding: '16px 20px 28px', position: 'relative', overflow: 'hidden' }}>
        ...
        <FadeUpAnimation delayMilliseconds={0}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
            ...
            >
              ...
              <div style={{ position: 'absolute', top: 8, right: 8, width: 8, height: 8, background: COLOR_DANGER, borderRadius: '50%', border: '1.5px solid rgba(21,61,76)' }} />
            </button>
          </div>
        </FadeUpAnimation>
      ...
      <div style={{ marginTop: -16, overflowX: 'auto', paddingBottom: 4 }}>
        <div style={{ display: 'flex', gap: 12, padding: '0 16px', width: 'max-content' }}>
          <FadeUpAnimation delayMilliseconds={100}>
            <div style={{ width: 148, borderRadius: 20, padding: 16, background: '#fff', boxShadow: '0 2px 12px rgba(0,0,0,0.07)', position: 'relative', overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 10, background: `${disciplineStatusColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={disciplineStatusColor} strokeWidth="2.5" strokeLinecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                </div>
                <span style={{ color: '#64748b', fontSize: 11, fontWeight: 700, lineHeight: 1.3 }}>Status Disiplin</span>
              </div>
              <div style={{ color: disciplineStatusColor, fontWeight: 900, fontSize: 24, fontFamily: 'Nunito', lineHeight: 1 }}>{disciplineStatusText}</div>
              <div style={{ marginTop: 4, color: '#94a3b8', fontSize: 11 }}>{totalViolationPoints} poin pelanggaran</div>
              <div style={{ marginTop: 8, height: 4, borderRadius: 4, background: '#f1f5f9' }}>
                <div style={{ width: `${Math.min(100, Math.max(10, totalViolationPoints))}%`, height: '100%', borderRadius: 4, background: disciplineStatusColor }} />
              </div>
            </div>
          </FadeUpAnimation>
          ...
          <FadeUpAnimation delayMilliseconds={150}>
            <div style={{ width: 148, borderRadius: 20, padding: 16, background: '#fff', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 10, background: '#E6F5F3', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 900, padding: '3px 8px', borderRadius: 8, background: '#E6F5F3', color: COLOR_PRIMARY, flexShrink: 0, marginLeft: 8, textTransform: 'capitalize' }}>{achievementItem.achievement_status || 'Terverifikasi'}</span>
                    </div>
